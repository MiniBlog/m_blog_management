...
public class UserController {
	...
	@PostMapping("/user.do")
	public void ajaxAddUser(TMbUser user, HttpServletResponse response) {
		log.info("SysUserController,ajaxAddUser:start");
		AjaxJson aj = new AjaxJson();
		try {
			String uid = sequenceService.getSequence("USER_ID");
			user.setUid(uid);
	...
	@PutMapping("/user.do")
	public void ajaxModifyUser(TMbUser user, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (StrUtil.isNotEmpty(org_pass)) {
				TMbUser userInfoInDb = userService.getUserInfoById(user.getId());
	...
	public void ajaxDeleteUser(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (StrUtil.isNotEmpty(id)) {
				TMbUser userInfoInDb = userService.getUserInfoById(NumberUtil.parseInt(id));
				if (userInfoInDb == null) {
					//用户不存在
					aj.setMsg("用户不存在");
					aj.setSuccess(false);
